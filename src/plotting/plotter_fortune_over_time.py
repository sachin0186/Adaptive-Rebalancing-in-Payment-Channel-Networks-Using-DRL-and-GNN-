...
def plot_total_fortune_over_time(outputs_directory, save_at_directory, filename):
    ...
    simulation_duration = traj.f_get('simulation_duration').f_get()
    ...
    for rebalancing_policy_index, rebalancing_policy in enumerate(par_rebalancing_policy_values):
        ...
        ax1.grid(True)
        ax1.set_xlim(0, simulation_duration)  # Set x-axis limit to simulation duration
...
if __name__ == '__main__':
    plot_total_fortune_over_time(outputs_directory="../../outputs", save_at_directory="../../outputs/figures/", filename='results_test')
